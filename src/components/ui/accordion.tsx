...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className={cn("flex flex-row justify-between items-center w-full")}>
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex items-center text-left text-base justify-between gap-[10px] py-2 transition-all [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
